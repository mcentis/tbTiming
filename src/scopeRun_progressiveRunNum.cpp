...
int findLastRun(const char* dirName){
  DIR* dir;
  dirent* dent;
  dir = opendir(dirName);
  if(dir == NULL){
  ...
  int runNum;
  int lastRun = -1; // if no runs are found -1 is returned
  
  std::string str;
  while((dent = readdir(dir)) != NULL){
    str = dent->d_name;
    
    if(str.find("run_") == 0 && str.find(".dat") == str.size() - 4){ // file names that starts with run_ and end with .dat
      runNum = atoi(str.substr(4, 6).c_str()); // extract the runNumber
...
int main(int argc, char* argv[]){
  ...
  int lastRun = findLastRun(argv[2]);
  char rep;
  if(lastRun == -1){
    std::cout << "There are no runs in the selected folder, are you sure you want to continue? [y to continue, any other key to abort]" << std::endl;
    std::cin >> rep;
    if(rep != 'y')
      return 0;
  }
  else{
    std::cout << "Last run found is " << lastRun << " launch run " << lastRun + 1 << " ? [y to continue, any other key to abort]" << std::endl;
    std::cin >> rep;
    if(rep != 'y')
      return 0;
  }
  ...
  sprintf(comm, "%s/run_%06i.dat", argv[2], lastRun + 1);
  ...
  sprintf(comm, "%s/run_%06i.txt", argv[2], lastRun + 1);
